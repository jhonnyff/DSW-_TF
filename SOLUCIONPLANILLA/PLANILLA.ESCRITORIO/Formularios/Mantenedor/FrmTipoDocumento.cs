...
{
    public partial class FrmTipoDocumento : FrmBase
    {
        public FrmTipoDocumento()
        {
            InitializeComponent();
        }
        TipoDocumentos obj = new TipoDocumentos();
        async void llenardatagrid()
        {
            try
            {
                ...
                response = await _httpClient.GetAsync($"{GlobalConstantes.ApiTipoDocumento}BusquedaTipoDocumento");
                ...
                using (HttpContent content = response.Content)
                {
                    ...
                    {
                        case 200:
                            List<TipoDocumentos> Lista = JsonConvert.DeserializeObject<List<TipoDocumentos>>(System.Convert.ToString(obj["data"]));
                            ...
                            if (Lista != null && Lista.Any())
                            {
                                Lista.OrderByDescending(r => r.Activo).ThenBy(r => r.Nombre).ToList().ForEach(list =>
                                {
                                    Dg1.Rows[Dg1.Rows.Add(
                                        list.IdTipoDocumento, list.Nombre, list.Activo, list.FecCreacion
                                        )].Tag = list;
            ...
            try
            {
                if (e.Button == MouseButtons.Right && Dg1.CurrentCell != null)
                {
                    if (Dg1.CurrentCell.ColumnIndex != 0)
                    {
                        ...
                        {
                            TipoDocumentos obj = Dg1.SelectedRows[0].Tag as TipoDocumentos;
                            if (obj != null)
                            {
        ...
        private void editarToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                obj = Dg1.SelectedRows[0].Tag as TipoDocumentos;
                if (obj != null)
                {
        ...
        private async void estadoToolStripMenuItem_ClickAsync(object sender, EventArgs e)
        {
            try
            {
                ...
                if (obj != null)
                {
                    ...
                    response = await _httpClient.GetAsync($"{GlobalConstantes.ApiTipoDocumento}CambiarEstado?id={obj.IdTipoDocumento}");
            ...
            try
            {
                ...
                switch (Accion_)
                {
                    case GlobalEnum._TipoAccion.Nuevo:
                        response = await _httpClient.PostAsJsonAsync($"{GlobalConstantes.ApiTipoDocumento}Insert", new TipoDocumentos { Nombre = Tnombre.Text });
                        break;
                    case GlobalEnum._TipoAccion.Modificar:
                        obj.Nombre = Tnombre.Text;
                        response = await _httpClient.PostAsJsonAsync($"{GlobalConstantes.ApiTipoDocumento}Update", obj);
                        break;
                }
