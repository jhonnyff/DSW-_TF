...
{
    public partial class FrmSistemaPensiones : FrmBase
    {
        public FrmSistemaPensiones()
        {
            InitializeComponent();
        }
        SistemaPensiones obj = new SistemaPensiones();
        async void llenardatagrid()
        {
            try
            {
                ...
                using (HttpContent content = response.Content)
                {
                    ...
                    {
                        case 200:
                            List<SistemaPensiones> Lista = JsonConvert.DeserializeObject<List<SistemaPensiones>>(System.Convert.ToString(obj["data"]));
                            ...
                            if (Lista != null && Lista.Any())
                            {
                                Lista.OrderByDescending(r => r.Activo).ThenBy(r => r.Nombre).ToList().ForEach(list =>
                                {
                                    Dg1.Rows[Dg1.Rows.Add(
                                        list.IdSistemaPension, list.Nombre, list.Activo, list.FecCreacion
                                        )].Tag = list;
            ...
            try
            {
                if (e.Button == MouseButtons.Right && Dg1.CurrentCell != null)
                {
                    if (Dg1.CurrentCell.ColumnIndex != 0)
                    {
                        ...
                        {
                            SistemaPensiones obj = Dg1.SelectedRows[0].Tag as SistemaPensiones;
                            if (obj != null)
                            {
        ...
        private void editarToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                obj = Dg1.SelectedRows[0].Tag as SistemaPensiones;
                if (obj != null)
                {
                    Tnombre.Text = obj.Nombre;
                    TAporte.Text = obj.Aporte.ToString();
                    TComision.Text = obj.Comision.ToString();
                    TPrima.Text = obj.Prima.ToString();
        ...
        private async void estadoToolStripMenuItem_ClickAsync(object sender, EventArgs e)
        {
            try
            {
                ...
                if (obj != null)
                {
                    ...
                    response = await _httpClient.GetAsync($"{GlobalConstantes.ApiSistemaPensiones}CambiarEstado?id={obj.IdSistemaPension}");
            ...
            try
            {
                ...
                switch (Accion_)
                {
                    case GlobalEnum._TipoAccion.Nuevo:
                        response = await _httpClient.PostAsJsonAsync($"{GlobalConstantes.ApiSistemaPensiones}Insert", new SistemaPensiones
                        {
                            Nombre = Tnombre.Text,
                            Aporte = ConvertForce.toDecimal(TAporte.Text),
                            Comision = ConvertForce.toDecimal(TComision.Text),
                            Prima = ConvertForce.toDecimal(TPrima.Text)
                        });
                        break;
                    case GlobalEnum._TipoAccion.Modificar:
                        obj.Nombre = Tnombre.Text;
                        response = await _httpClient.PostAsJsonAsync($"{GlobalConstantes.ApiSistemaPensiones}Update", obj);
                        break;
                }
