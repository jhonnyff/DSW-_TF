...
{
    public partial class FrmGeneros : FrmBase
    {Generos obj=new Generos();
        ...
        async void llenardatagrid()
        {
            try
            {
                ...
                response = await _httpClient.GetAsync($"{GlobalConstantes.ApiGenero}BusquedaGeneros");
                ...
                using (HttpContent content = response.Content)
                {
                    ...
                    {
                        case 200:
                            List<Generos> Lista = JsonConvert.DeserializeObject<List<Generos>>(System.Convert.ToString(obj["data"]));
                            ...
                            if (Lista != null && Lista.Any())
                            {
                                Lista.OrderByDescending(r => r.Activo).ThenBy(r => r.Nombre).ToList().ForEach(list =>
                                {
                                    Dg1.Rows[Dg1.Rows.Add(
                                        list.IdGenero, list.Nombre, list.Activo, list.FecCreacion
                                        )].Tag = list;
        ...
        private void editarToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                obj = Dg1.SelectedRows[0].Tag as Generos;
                if (obj != null)
                {
        ...
        private async void estadoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (obj != null)
                {
                    ...
                    response = await _httpClient.GetAsync($"{GlobalConstantes.ApiGenero}CambiarEstado?id={obj.IdGenero}");
            ...
            try
            {
                ...
                switch (Accion_)
                {
                    case GlobalEnum._TipoAccion.Nuevo:
                        response = await _httpClient.PostAsJsonAsync($"{GlobalConstantes.ApiGenero}Insert", new Generos { Nombre = Tnombre.Text });
                        break;
                    case GlobalEnum._TipoAccion.Modificar:
                        obj.Nombre = Tnombre.Text;
                        response = await _httpClient.PostAsJsonAsync($"{GlobalConstantes.ApiGenero}Update", obj);
                        break;
                }
            ...
            try
            {
                if (e.Button == MouseButtons.Right && Dg1.CurrentCell != null)
                {
                    if (Dg1.CurrentCell.ColumnIndex != 0)
                    {
                        ...
                        {
                            Generos obj = Dg1.SelectedRows[0].Tag as Generos;
                            if (obj != null)
                            {
