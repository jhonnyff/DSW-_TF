...
{
    public partial class FrmEstadoCiviles : FrmBase
    {
        EstadosCiviles obj = new EstadosCiviles();
        public FrmEstadoCiviles()
        {
            InitializeComponent();
        }
        async void llenardatagrid()
        {
            try
            {
                ...
                response = await _httpClient.GetAsync($"{GlobalConstantes.ApiEstadoCivil}BusquedaEstadoCivil");
                ...
                using (HttpContent content = response.Content)
                {
                    ...
                    {
                        case 200:
                            List<EstadosCiviles> Lista = JsonConvert.DeserializeObject<List<EstadosCiviles>>(System.Convert.ToString(obj["data"]));
                            ...
                            if (Lista != null && Lista.Any())
                            {
                                Lista.OrderByDescending(r => r.Activo).ThenBy(r => r.Nombre).ToList().ForEach(list =>
                                {
                                    Dg1.Rows[Dg1.Rows.Add(
                                        list.IdEstadoCivil, list.Nombre, list.Activo, list.FecCreacion
                                        )].Tag = list;
            ...
            try
            {
                ...
                switch (Accion_)
                {
                    case GlobalEnum._TipoAccion.Nuevo:
                        response = await _httpClient.PostAsJsonAsync($"{GlobalConstantes.ApiEstadoCivil}Insert", new Cargos { Nombre = Tnombre.Text });
                        break;
                    case GlobalEnum._TipoAccion.Modificar:
                        obj.Nombre = Tnombre.Text;
                        response = await _httpClient.PostAsJsonAsync($"{GlobalConstantes.ApiEstadoCivil}Update", obj);
                        break;
                }
            ...
            try
            {
                if (e.Button == MouseButtons.Right && Dg1.CurrentCell != null)
                {
                    if (Dg1.CurrentCell.ColumnIndex != 0)
                    {
                        ...
                        {
                            EstadosCiviles obj = Dg1.SelectedRows[0].Tag as EstadosCiviles;
                            if (obj != null)
                            {
        ...
        private void editarToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                obj = Dg1.SelectedRows[0].Tag as EstadosCiviles;
                if (obj != null)
                {
        ...
        private async void estadoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                obj = Dg1.SelectedRows[0].Tag as EstadosCiviles;
                if (obj != null)
                {
                    ...
                    response = await _httpClient.GetAsync($"{GlobalConstantes.ApiEstadoCivil}CambiarEstado?id={obj.IdEstadoCivil}");
        ...
        private async void FrmEstadoCiviles_Load(object sender, EventArgs e)
        {
            try
            {
                llenardatagrid();
