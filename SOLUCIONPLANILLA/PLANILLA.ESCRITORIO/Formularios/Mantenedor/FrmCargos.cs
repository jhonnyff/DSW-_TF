...
{
    public partial class FrmCargos : FrmBase
    {
        ...
        async void llenardatagrid()
        {
            try
            {
                ...
                response = await _httpClient.GetAsync($"{GlobalConstantes.ApiCargo}BusquedaCargos");
                ...
                using (HttpContent content = response.Content)
                {
                    ...
                    {
                        case 200:
                            List<Cargos> Lista = JsonConvert.DeserializeObject<List<Cargos>>(System.Convert.ToString(obj["data"]));
                                ...
                                Lista.OrderByDescending(r => r.Activo).ThenBy(r => r.Nombre).ToList().ForEach(list =>
                            {
                                Dg1.Rows[Dg1.Rows.Add(
                                    list.IdCargo, list.Nombre, list.Activo, list.FecCreacion
                                    )].Tag = list;
            ...
            try
            {
                if (e.Button == MouseButtons.Right && Dg1.CurrentCell != null)
                {
                    if (Dg1.CurrentCell.ColumnIndex != 0)
                    {
                        ...
                        {
                            Cargos obj = Dg1.SelectedRows[0].Tag as Cargos;
                            if (obj != null)
                            {
        ...
        private async void BGrabar_ClickAsync(object sender, EventArgs e)
        {
            try
            {
                ...
                switch (Accion_)
                {
                    case GlobalEnum._TipoAccion.Nuevo:
                        response = await _httpClient.PostAsJsonAsync($"{GlobalConstantes.ApiCargo}Insert", new Cargos { Nombre = Tnombre.Text });
                        break;
                    case GlobalEnum._TipoAccion.Modificar:
                        obj.Nombre = Tnombre.Text;
                        response = await _httpClient.PostAsJsonAsync($"{GlobalConstantes.ApiCargo}Update", obj);
                        break;
                }
        ...
        private void editarToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                obj = Dg1.SelectedRows[0].Tag as Cargos;
                if (obj != null)
                {
        ...
        private async void estadoToolStripMenuItem_ClickAsync(object sender, EventArgs e)
        {
            try
            {
                ...
                if (obj != null)
                {
                    ...
                    response = await _httpClient.GetAsync($"{GlobalConstantes.ApiCargo}CambiarEstado?id={obj.IdCargo}");
